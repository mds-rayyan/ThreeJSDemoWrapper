...
export default class Transition {
    constructor(sceneA, sceneB) {
        ...
        this.quadgeometry = new THREE.PlaneGeometry(window.innerWidth, window.innerHeight);
        ...
        this.render = function (delta) {
            ...
            if (this.transitionParams.animateTransition) {
                var t = (1 + Math.sin(this.transitionParams.transitionSpeed * this.clock.getElapsedTime() / Math.PI)) / 2;
                this.transitionParams.transition = THREE.Math.smoothstep(t, 0.3, 0.7);

                // Change the current alpha texture after each transition
                if (this.transitionParams.loopTexture && (this.transitionParams.transition == 0 || this.transitionParams.transition == 1)) {
                    if (this.needChange) {
                        this.transitionParams.texture = (this.transitionParams.texture + 1) % this.textures.length;
                        this.quadmaterial.uniforms.tMixTexture.value = this.textures[this.transitionParams.texture];
                        this.needChange = false;
                    }
                }
                else
